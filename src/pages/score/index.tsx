...
export default () => {
  ...
  const columns: ProColumns<API.ScoreVO>[] = [
    ...
    },
    {
      ...
      sorter: true,
      render: (text) => {
        if (text >= 270) { // 假设总分三门300分时是高分
          return <Badge count={text} overflowCount={999} style={{ backgroundColor: '#2db7f5' }} offset={[0, 0]} />;

        } else if (text < 180) { // 假设总分180以下为低分
          return <Badge count={text} overflowCount={999} style={{ backgroundColor: '#f5222d' }} offset={[0, 0]} />;
        }
        return <span>{text}</span>;
      },
    },
    ...
    },
    {
      ...
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [<Link to={`/score/detail?id=${record.id}`}>修改</Link>],
    },
    {
      title: '学生信息',
      width: 100,
      ...
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [<Link to={`/student?studentId=${record.studentId}`}>学生信息</Link>],
    },
  ];
